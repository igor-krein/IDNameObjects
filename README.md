...
When we talk abuout JQuery, all this could be achieved with the help of the <a href="https://select2.github.io/">Select2</a>. On the server side, you should create a proper query to get the data requested, convert it to Select2-format and return the result.
...
The method that gets the ajax request would probably look like this:
    ...
    [HttpGet]
    public ActionResult GetBooks(string search, int pageSize, int pageNum)
    {
        ...
        var books = db.Books
                        ...
                        .Skip(pageSize * pageNum)             // go to the proper virtual "page"
...
Before showing you examples, I have to remind you that we don't have to pass all the data to the client. All we need is to get currently selected item (or, in case of multi-select list, items) only. All the rest would be drawn on user's demand with the help of GetBooks method.

Let's start with [single-]select list (suppose we need to choose one favorite book). Let's say, there is such a book already chosen (and stored in DB) and we want to let the user change his/her choice. So, we do this:
...
Suppose, we want the default order to be like this: first by year, and afterwards by title. We could do this by adding to our model a class method:
    ...
    public static Expression<Func<T, object>> TypeHereAnyNameYouWant<T>()
        where T : Book
    {
        return b => b.Year.ToString() + b.Name;
    }
...
But if you plan to derive from the Book class and use the same ordering method for it you should use a generic version.
...
__Third option__: use explicit ordering. Most of the API methods support optional lambda-expression parameter like this:
    ...
    var select2result = db.Books.ToSelect2PageResult(search, pageNumber, pageSize, b => b.Year.ToString() + b.Name);
	// ...
	ViewBag.BooksList = db.Books.ToMultiSelectList(ids, b => b.Year.ToString() + b.Name);
...
Generally, there are only two these methods currently supported. But if, for some reason, you want to change default name filtering behaviour, you could define yours like this (__second option__):
        ...
        where T : Book
    {
        ...
        return b => b.Name.Equals(filterString);
    }
...
Our Name property is combined from three other properties and, which is important, is not mapped to the database. It means, in the DB table, there are fields FirstName, MiddleName and LastName, but there is no field for FullName. In most cases, it is OK, but if you try to use FullName in LINQ expressions you'll get a run-time error. As a result, IDNameObjects API would fail to apply default filter and order methods which is annoying.
...
Up until now what we actually use is some high-level API methods (for creating select lists and for preparing data for Select2). These methods are belonged to Web API part (namespace IDNameObjects.Mvc).
...
These methods use default filter and order methods and put the result in the list
	...
	var selectedBooks = db.Books.QuickList(search, pageNumber, b => b.Year.ToString() + b.Name);
...
Note: the selectedBooksQuery variable would be of type IQueryable&lt;Book&gt;, so you could use it, say, with stabdard EF fluent API.
...
Well, maybe you won't use these much, but note that you can use them along with the standard fluent API:
    ...
    var selectedBooks = db.Books.WhereIDsNotIn(ids).WhereNameContains(search).OrderBy(b => b.Name).ToList();
...
This should work well on .NET 4.6 and with MSSQL. Haven't tested it on .NET Core yet.